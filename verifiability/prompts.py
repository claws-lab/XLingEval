...
def prompt_verifiability(question: str, response: str, language:str):
    ...
    if language == "Chinese":
        ...
        prompt = f"Use one English word (either 'Yes' or 'No'. Do not give any reasons) to answer me whether the following response is a factually correct answer (or diagnosis) to the following question (or scenario). Both the question and answer are delimited by triple backticks. \nQuestion: ```{question}``` \nResponse: ```{response}```."
