...
def map_prediction_to_binary_MedAlpaca(x, return_string=True, return_unknown=False):
    ...
    if isinstance(x, str) and (any([segment in x for segment in ANSWER_MAP["No"]]) and any([segment in x
        for segment in ANSWER_MAP["Yes"]])):
        print(x)
        return "Unknown" if return_string else -1


    # Only "Yes" is in the answer
    elif isinstance(x, str) and any([segment in x for segment in ANSWER_MAP["Yes"]]):
        return "Yes" if return_string else 1

    # Only "No" is in the answer
    elif isinstance(x, str) and any([segment in x for segment in ANSWER_MAP["No"]]):
...
def map_prediction_to_binary(x, return_string=True, return_unknown=False):
   ...
   """
    ...
    if isinstance(x, str) and (any([x.capitalize().startswith(prefix) for prefix in ANSWER_MAP["No"]]) or ("no" in x.lower() and "yes" not in x.lower())):
        return "No" if return_string else 0

    elif isinstance(x, str) and (any([x.capitalize().startswith(prefix) for prefix in ANSWER_MAP["Yes"]] or ("yes" in x.lower() and "no" not in x.lower()))):
        return "Yes" if return_string else 1
